...
curl -sS -u $HARPER_USERNAME:$HARPER_PASSWORD \
     -X POST $HARPER_URL/<PIPELINE_ID_PASCAL>Run \
     -d '{}'
```
...
- `curl $HARPER_URL/<PIPELINE_ID_PASCAL>Run` (GET) → returns registry row, but only after step 2 below
...
```bash
curl -sS -u $HARPER_USERNAME:$HARPER_PASSWORD \
     "$HARPER_URL/<TARGET_TABLE>/?limit=3"
```
...
3. **How the funnel consumes it** — the REST URL(s) to hit. For table `ContractorRegistration`:
   - `GET $HARPER_URL/ContractorRegistration/?limit=100`
   - `GET $HARPER_URL/ContractorRegistration/?filter={"state":"CO"}`
